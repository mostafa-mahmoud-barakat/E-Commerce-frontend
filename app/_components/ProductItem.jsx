...
function ProductItem({product}) {
  return (
    <Link href={`/product-details/${product?.id}`}  >
      {/* className='p-1 border-teal-400 hover:border  hover:shadow-md rounded-lg hover:cursor-pointer' */}
       <div className='p-1 border-teal-400 hover:border  hover:shadow-md rounded-lg hover:cursor-pointer'>
          <Image src={product?.attributes?.banner?.data?.
            attributes?.url} 
            alt='banner-card' width={400} height={350}
            ...
            <div>
                ...
                <h2 className='text-[10px] text-gray-400 flex gap-1 items-center'><List className='w-4 h-4'/>{product?.attributes?.category}</h2>
            </div>
