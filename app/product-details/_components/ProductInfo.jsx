...
function ProductInfo({product}) {
  ...
  return (
    <div>
        ...
        <h2 className='text-[11px] text-gray-500 flex gap-2 mt-2 items-center'> {product?.attributes?.instantDelivery? 
            <BadgeCheck className='text-green-500 w-5 h-5'/>: <AlertOctagon/>}Eligible For Instant Delovery</h2>
        <h2 className='text-[32px] text-primary mt-3'>${product?.attributes?.price}</h2>
        <button onClick={()=>handleAddToCart()} className='flex gap-2 bg-primary hover:bg-teal-600 rounded-lg p-3 text-white'>
